...
#define endl '\n'
#define file_locate "D:/SMMS/data.txt"
using namespace std;
...
void check_file(){
	filesystem::path directoryPath("D:/SMMS");
	...
	ifstream fileExist(file_locate);
	if(!(fileExist.good())){
		ofstream file_create(file_locate);
...
#define bat_locate "D:/SMMS/link.bat"
void check_bat(){
	ifstream fileExist(bat_locate);
	if(!(fileExist.good())){
		ofstream file_create(bat_locate);
		if(file_create.is_open()){
			file_create<<"start \"\" \"C:\\Users\\Administrator\\AppData\\Local\\Programs\\Microsoft VS Code\\Code.exe\""<<endl;
			file_create<<"D:"<<endl<<"cd SMMS"<<endl;
			file_create<<"start \"\" \"D:\\SMMS\\smms.exe\"";
			file_create.close();
		}else	cerr<<"error in create new file data"<<endl,exit(0);
	}
}
signed main() 
{
	...
	ofstream fout(file_locate);
	fout<<temp_date.hour<<" "<<temp_date.day<<" "<<temp_date.month<<" "<<temp_date.year<<endl;
	fout<<money;
	FOR(i,1,60)	cout<<"=";	cout<<endl;
	cout<<"Account savings money balance: "<<money<<"k vnd";
	cout<<endl;
	system("pause");
	fout.close();
	//cout<<"=                                                          ="<<endl;
